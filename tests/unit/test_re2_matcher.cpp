...
using shield::Re2PatternMatcher;

static Re2PatternMatcher::Rule make_rule(
    const std::string& id,
    const std::string& pattern,
    int                mask_group,
    const std::string& replacement = "***")
{
    Re2PatternMatcher::Rule r;
    r.id          = id;
...
// UT-06: Password 마스킹
TEST(Re2PatternMatcherTest, PasswordMasking) {
    Re2PatternMatcher m;
...
TEST(Re2PatternMatcherTest, BearerTokenMasking) {
    Re2PatternMatcher m;
...
// UT-08: 이메일 마스킹
TEST(Re2PatternMatcherTest, EmailMasking) {
    Re2PatternMatcher m;
...
TEST(Re2PatternMatcherTest, NoMatchReturnsNullopt) {
    Re2PatternMatcher m;
...
// UT-10: 복수 패턴 동시 적용
TEST(Re2PatternMatcherTest, MultipleRulesApplied) {
    Re2PatternMatcher m;
...
// UT-23: 한국 전화번호 마스킹
TEST(Re2PatternMatcherTest, KoreanPhoneMasking) {
    Re2PatternMatcher m;
...
// UT-24: 신용카드 번호 마스킹
TEST(Re2PatternMatcherTest, CreditCardMasking) {
    Re2PatternMatcher m;
...
// 추가: 빈 룰셋 → nullopt
TEST(Re2PatternMatcherTest, EmptyRuleSet) {
    Re2PatternMatcher m;
