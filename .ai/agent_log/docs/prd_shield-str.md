...
## 추가 요구사항 (v1.2.0)
- **Stdlib-Only 모드 지원**: 외부 라이브러리(Google RE2, simdjson) 의존성 없이 C++ 표준 라이브러리(`std::regex`)만으로 독립적으로 동작할 수 있는 모드를 지원하여 사용자가 취사 선택할 수 있게 한다.
- **성능 시각화 및 대시보드 (GitHub Pages)**: `RE2` 기반 고성능 모드와 `std::regex` 기반 독립 모드의 성능 벤치마크 결과를 시각적으로 보여주고, 사용자가 직접 문자열을 입력해 마스킹 처리 전후(diff)를 확인 할 수 있는 대시보드를 제공한다.
- **가이드 문서화**: 외부 시스템에서 쉽게 적용할 수 있도록 API 활용 예시 및 빌드/통합 가이드 문서(`docs/api-reference.md`, `README.md`)를 작성한다.
